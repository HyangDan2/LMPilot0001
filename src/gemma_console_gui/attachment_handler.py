...
def _read_image(path: Path, image_mode: ImageMode) -> str:
    try:
        ...
        raise AttachmentError("Image preprocessing requires the 'Pillow' package.") from exc
...
def _ocr_image(image, path: Path) -> str:
    ...
    try:
        ...
        return (
            "OCR text:\n"
            "OCR unavailable: install the Python package 'pytesseract' in the same environment as the GUI, "
            "and install the native Tesseract OCR engine. If Tesseract is not on PATH, set TESSERACT_CMD "
